 ...
 */
void UKF::ProcessMeasurement(MeasurementPackage reading) {
  float EPS =  0.001;
  if (!is_initialized_) {
    switch (reading.sensor_type_) {
      case MeasurementPackage::LASER:
        x_ << reading.raw_measurements_(0), reading.raw_measurements_(1), 0.0, 0.0, 0.0;
      ...
      case MeasurementPackage::RADAR:
        x_ << reading.raw_measurements_(0) * cos(reading.raw_measurements_(1)),
              reading.raw_measurements_(0) * sin(reading.raw_measurements_(1)),
              0.0, 0.0, 0.0;
        break;
      }
       // Save the initial timestamp for dt calculation
       time_us_ = reading.timestamp_;
       // Done initializing, no need to predict or update
       is_initialized_ = true;
       return;
  }
  double dt = (reading.timestamp_ - previous_timestamp_) / 1.0e6;
...
void UKF::Update(MeasurementPackage reading) {

  if (reading.sensor_type_ == MeasurementPackage::RADAR  && use_radar_) {
    VectorXd z_pred = VectorXd(3);
    MatrixXd Zsig_pred = MatrixXd(3,15);

    //generate covariance matrix for radar measurements
    ...
    //generate covariance matrix for laser measurements
...
void UKF::UpdateLidar(const MatrixXd& Xsig_pred,
  ...
  //residual
  VectorXd z_diff = reading.raw_measurements_ - z_pred;
...
void UKF::PredictSigmaPoints(const MatrixXd& Xsig_aug,
                             const double dt,
                             MatrixXd* Xsig_out) {
